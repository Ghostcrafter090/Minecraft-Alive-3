...
execute at @s run scoreboard players set @e[tag=!notick,tag=dmain,type=marker] enderBool 1
execute at @s run summon marker ~ ~ ~ {Tags:["ender_key_spawn","effectend"]}
execute at @s run summon marker ~ ~ ~ {Tags:["ender_key_spawn_site","effectend"]}
execute at @s run summon wither ~ 500 ~ {NoGravity:1b,Silent:1b,Invulnerable:1b,CustomNameVisible:1b,NoAI:1b,Tags:["magMark","effectend"],CustomName:'{"text":"End Key Spawn Site"}'}
...
execute at @s run scoreboard players set @p endKeySpawn 1

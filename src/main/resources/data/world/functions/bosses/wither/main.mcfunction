...
execute as @e[type=item,tag=fire_key_shard] at @s if block ~ ~ ~ minecraft:water_cauldron[level=3] if block ~ ~-1 ~ minecraft:soul_campfire[lit=true] if entity @e[type=item,limit=1,distance=0..1,nbt={Item:{id:"mca:water_key",Count:1b}}] run particle minecraft:soul_flame ~ ~ ~ 1 10 1 0.1 10 force
